...
class SelfAttention(tf.keras.layers.Layer):
    def __init__(self,
        ...
        attention_initializer = _glorot_initializer(self.d_model,
                                                    self.d_model)
        ...
        if self.head_initialization == 'one_sided':
            head_pos = tf.range(1./(2.*self.n_head), 1., 1./self.n_head, dtype=tf.float32)
        elif self.head_initialization == 'two_sided':
            head_pos = tf.range(-1.+1./self.n_head, 1., 2./self.n_head, dtype=tf.float32)
        else:
            assert False, "head_initialization should be one of ['one_sided', 'two_sided']"
